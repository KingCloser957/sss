...
class VPNManager {
    ...
    func loadManager() {
        NETunnelProviderManager.loadAllFromPreferences { (managers, error) in
            ...
            if let manager = managers?.first {
                ...
                self.manager?.localizedDescription = "悟空加速器"
            }
    ...
    func newConfiguration() -> NETunnelProviderProtocol {
        ...
        tunnelProtocol.serverAddress = "164.92.94.19"
        //指定network extension 确保bundleIdentifier和network extension的id一致
        tunnelProtocol.providerBundleIdentifier = Bundle.main.bundleIdentifier!.appending(".tunnel")
